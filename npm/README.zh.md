...
无需任何先决条件的 **npx** 安装 [`prism`](https://github.com/mcp-tool-shop-org/prism-verify) 命令行工具——一种运行时大型语言模型（LLM）验证器（针对不同模型系列，去除推理过程，采用多重视角，并使用签名 Ed25519 收据）。
...
这是一个对 [`@mcptoolshop/npm-launcher`](https://github.com/mcp-tool-shop-org/npm-launcher) 的轻量级封装：它从 [prism-verify GitHub 发布](https://github.com/mcp-tool-shop-org/prism-verify/releases) 下载特定平台的 `prism` 二进制文件，**验证其 SHA256 校验和**，将其缓存到 (`~/.cache/mcptoolshop/prism/<version>/`)，然后运行。网络访问仅限于 HTTPS 协议，连接到 GitHub；不收集遥测数据，也不存储任何凭据。
