...
def create_app(
    *,
    ...
    # The cache holds the full response payload so a replay returns a byte-identical body.
    idempotency: dict[str, tuple[str, dict[str, Any] | None]] = {}
    ...
    async def verify(body: VerifyHttpRequest, request: Request, response: Response) -> Response:
        ...
        if idem_key is not None:
            ...
            if cached is not None:
                cached_fp, cached_payload = cached
                ...
                return _json(cached_payload, rate_headers)
        ...
        if prefer_async:
            ...
            return _json(
                {"status": "accepted", "delivery": "webhook", "webhook": body.webhook},
                rate_headers,
                status=202,
            )

        if idem_key is not None:
            idempotency[idem_key] = (fingerprint, None)
        result = await engine.verify(_to_core_request(body))
        if isinstance(result, VerifyError):
            if idem_key is not None:
        ...
        if idem_key is not None:
            idempotency[idem_key] = (fingerprint, payload)
