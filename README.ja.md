...
エージェントのワークフローに対する実行時の裁定サービス。異なるファミリー間で、推論機能を排除し、複数の視点から検証を行い、再現可能な証拠を記録します。**[ランディングページとハンドブックはこちら →](https://mcp-tool-shop-org.github.io/prism-verify/)**
...
Prismは、常に呼び出し元のモデルファミリーとは異なるモデルファミリーを使用して検証を行うため（ロック1）、少なくとも1つの代替モデルファミリープロバイダーを設定してください。署名用の秘密鍵を設定します（または、ローカル環境でのテスト用に`PRISM_DEV=1`を設定します）。これにより、検証結果を記録できます。
