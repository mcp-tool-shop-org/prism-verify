...
エージェントのワークフローに対する、実行時の判断サービス。異なるモデル群を使用し、推論機能を排除した上で、再現可能なログを用いて、コード、ツールの呼び出し、引用、および応答における**迎合的な行動**を多角的に検証します。**[ランディングページとハンドブックはこちら →](https://mcp-tool-shop-org.github.io/prism-verify/)**
...
**引用**に関する検証において、LLMの根拠性チェックの前に、階層化された検証プロセスが実行されます。各決定的な段階では、証明できることのみを受け入れ、それ以外は却下します。

- **存在性検証層** — 実際のarXiv/Crossrefからの情報を取得し、捏造された識別子は検証対象から除外します。
- **数値/単位検証層** — パーセンテージの入れ替え、単位スケールのずれ（42ミリ秒 vs マイクロ秒）、または比較方向の誤り（5.0 < 5.8 ≠ 「超過」）などを算術的に検出します。
- **根拠性チェック** — 取得した概要と、異なるモデルファミリーに属する、推論能力を制限されたLLMを比較します。
- **直交NLI検証層** *(オプション、`PRISM_NLI_FLOOR`)* — エンコーダーNLIクロスエンコーダーが、LLMが「支持」したものの、メカニズムが異なる別のモデルでは裏付けられない場合に、その結果を却下します。

### 独自の検証ツールをご利用ください

根拠性チェックは、ホストされたAPIの代わりに、**お客様がホストする**モデルに対して実行できます。`PRISM_LOCAL_VERIFIER_ENDPOINT`を通じてオプションで設定し、異なるモデルファミリーを使用し、お客様がホストする検証ツールにフォールバックします。最も頻繁に実行されるチェックは、1回の実行あたり費用がかからず、証拠はローカルに保存されます。オプションのキャプチャシンク（`PRISM_HARVEST_PATH`）は、`(主張、証拠、結果)`のトリプルを記録し、それを使用してモデルをトレーニングできます。詳細については、[ハンドブック](https://mcp-tool-shop-org.github.io/prism-verify/handbook/local-verifier/)をご覧ください。

### 迎合的な行動（応答の検証）

コード、ツールの呼び出し、引用に加えて、Prismはモデルの**応答**を、*後退的*な**迎合的な行動**に対して評価します。つまり、正しいことよりも、ユーザーが聞きたいことを伝える（誤った前提を肯定したり、わずかな反論で正しい答えを放棄したりする）ことです。異なるモデル群を使用し、推論機能を排除した上で微調整された専門モデルを`sycophancy`の検証機能として実行します。これは`PRISM_SYCOPHANCY_ENDPOINT`を通じてオプションで有効にできます。**エラーが発生した場合でも、常に「判断しない」という結果を出力します**（決して無言で「迎合的ではない」とは表示しません）。*正しい*ユーザーの意見に同意したり、十分な証拠に基づいた反論を受け入れたりすることは、誠実であり、迎合的な行動ではありません。詳細は[ハンドブック](https://mcp-tool-shop-org.github.io/prism-verify/handbook/)をご覧ください。
